...
For example, a Bar chart. There is an udnerlying calculation happening - it counts the rows and plots THAT number. 
...
```{r}
#same exacy code, but with a y aes mapped
# dat_ozone_long %>% 
#   filter(DateTime >= as.Date('2017-05-01') & DateTime <= as.Date('2017-10-01')) %>% 
...
###### add discussion of Y axes, and free_y. Are you contrasting between facets? Don't lie!!
...
Another example of a faceted chart. This time, we used mutate to create some new variables, then faceted on those variables (month and day of month).
...
plot_pols = function(df, pol){
    for(pol in Pollutants){
        p = ggplot(NEI_totals_long %>% 
